...
def handle_all(message):
    ...
    if message.text == "📞 Get Number":
        ...
        if found:
            ...
            # ✅ নতুন মেসেজ (Requirement 3)
            bot.reply_to(message, "✅ **New numbers stock added successfully!**")
            
            # দেশ অনুযায়ী স্টক দেখানো
            stock_summary = "📊 **Current Stock Summary:**\n\n"
            for country, numbers in sorted(curr_db.items()):
                if numbers:
                    stock_summary += f"{country} → **{len(numbers)}** numbers\n"
            
            if stock_summary.strip():
                bot.send_message(message.chat.id, stock_summary)
