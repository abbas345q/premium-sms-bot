...
# threaded=True করলে callback fast response করবে
...
def main_keyboard():
    ...
    markup.row(
        types.KeyboardButton("🌍 Available Countries")
    )
...
def admin_settings(message):
    ...
    # ⚠️ এখানে শুধু Broadcast Button remove করা হয়েছে
    markup.add(
        types.InlineKeyboardButton(
            "💵 Set Refer Bonus",
            callback_data="conf_ref"
        ),

        types.InlineKeyboardButton(
            "🏧 Set Min Withdraw",
            callback_data="conf_with"
        ),

        types.InlineKeyboardButton(
            "⚙️ Manage Channels",
            callback_data="conf_chan"
        ),

        types.InlineKeyboardButton(
            "📊 Export Available Stock",
            callback_data="conf_export"
        ),

        types.InlineKeyboardButton(
            "🗑️ Clear Stock",
            callback_data="conf_clear"
        )
    )

    text = (
        f"🛠 Admin Control Panel\n\n"
        f"💰 Refer Bonus: {config['ref_bonus']} BDT\n"
...
def handle_all(message):
    ...
    if message.text == "📞 Get Number":
        ...
        bot.send_message(
            message.chat.id,
            f"💳 **Current Balance:** {u_data.get('balance', 0.0)} BDT"
        )
        ...
        bot.send_message(
            message.chat.id,
            f"🎁 **Refer Link:** https://t.me/{bot_user}?start={uid}"
        )
        ...
        if found:
            ...
            for r in found:
                ...
                try:
                    name = geocoder.description_for_number(
                        phonenumbers.parse(clean_r),
                        "en"
                    )
                ...
                c_name = (
                    f"{flag} {name}"
                    if name != "Unknown"
                    else f"📍 Zone +{clean_r[1:4]}"
                )

                if c_name not in curr_db:
                    curr_db[c_name] = []
                ...
                if clean_r not in curr_db[c_name]:
                    ...
                    # country count
                    country_added[c_name] = country_added.get(c_name, 0) + 1

            save_data(DB_FILE, curr_db)

            # সুন্দর stock added message
            msg = (
                f"✅ New Number Stock Added\n\n"
                f"📦 Total Added: {added}\n\n"
            )

            for cname, total in sorted(country_added.items()):
                msg += f"{cname} ➜ {total}\n"
...
def handle_query(call):
    ...
    if call.data == "verify_join":
        ...
        if country in curr_db and len(curr_db[country]) >= 1:
            ...
            for _ in range(take_count):
                ...
                if curr_db[country]:
                    ...
                    users[uid]["active_numbers"].append({
                        "number": raw_num,
                        "country": country
                    })
            ...
            # ⚡ Fast Native Inline Keyboard
            custom_markup = types.InlineKeyboardMarkup(row_width=1)
            ...
            for num in delivered_numbers:
                ...
                btn = types.InlineKeyboardButton(
                    ...
                    copy_text=types.CopyTextButton(str(num))
                )

                custom_markup.add(btn)
        ...
        if not active_stock:
            ...
            bot.answer_callback_query(
                call.id,
                "❌ Database stock is completely empty!",
                show_alert=True
            )
        ...
        try:
            ...
            with open(filename, "w", encoding="utf-8") as f:
                ...
                for country, numbers in sorted(active_stock.items()):
                    ...
                    f.write(
                        f"[{country}] - Available: {len(numbers)}\n"
                    )
        ...
        bot.edit_message_text(
            "🗑️ **Select country to clear stock:**",
            chat_id,
            message_id,
            reply_markup=markup
        )
        ...
        for i, ch in enumerate(config.get('channels', [])):
            ...
            markup.add(
                types.InlineKeyboardButton(
                    f"🗑️ Delete {ch['username']}",
                    callback_data=f"delch_{i}"
                )
            )
        ...
        bot.register_next_step_handler(
            msg,
            lambda m: update_cfg(m, 'ref_bonus')
        )
        ...
        bot.register_next_step_handler(
            msg,
            lambda m: update_cfg(m, 'min_withdraw')
        )
